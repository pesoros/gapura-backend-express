...
module.exports = {
    ...
    getAll: (offset, limit, sort, sortBy, search, category, admin) => {
        return new Promise((resolve, reject) => {
            ...
            const query = `SELECT art.categories_id, art.title, art.slug, art.label, art.sublabel, art.image, art.createdAt, updatedAt, cat.title as categories FROM articles art, categories cat WHERE art.title LIKE CONCAT(?,'%') AND art.categories_id = cat.id ${adminq} ${categoryq}`
